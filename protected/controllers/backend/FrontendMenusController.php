...
class FrontendMenusController extends CController
{
	...
	{
		$this->redirect($this->_backendPath.'frontendMenus/manage');
    }	  
    ...
    public function manageAction($pid = 0)
    {
		...
		Website::prepareBackendAction('view', 'frontend_menu', 'dashboard/index');
		...
		$this->_view->render($this->_backendPath.'frontendMenus/manage');
    }
    ...
    public function addAction($pid = 0)
    {
    	...
    	$this->_view->render($this->_backendPath.'frontendMenus/add');
    }    
    ...
    public function editAction($id = 0)
    {
    	...
    	$this->_view->render($this->_backendPath.'frontendMenus/edit');
    }    
    ...
    public function changeStatusAction($id, $page = 0)
    {
        ...
        $this->redirect($this->_backendPath.'frontendMenus/manage'.$parentMenuPart.(!empty($page) ? '?page='.(int)$page : 1));
    }
    ...
    public function deleteAction($id = 0, $pid = 0)
    {
		...
		$this->redirect($this->_backendPath.'frontendMenus/manage'.(!empty($pid) ? '/pid/'.(int)$pid : ''));
    }
    ...
    private function _getPagesLinks()
    {
        ...
        if(Modules::model()->isInstalled('cms')){
			$pages = \Modules\Cms\Models\Pages::model()->findAll(array('condition'=>'publish_status = 1', 'order'=>'sort_order ASC'));
