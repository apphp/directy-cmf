<?php
	...
	$this->_activeMenu = $backendPath.'frontendMenus/';
    $breadCrumbs = array(
        array('label'=>A::t('app', 'General'), 'url'=>$backendPath.'dashboard'),
    );
	if($parentId && $parentName != ''){
		$breadCrumbs[] = array('label'=>A::t('app', 'Frontend Menu'), 'url'=>$backendPath.'frontendMenus/manage');
...
<div class="bloc">
    <div class="title">
	<?php
		echo (($parentName != '') ? $parentName.' <a class="back-link" href="'.$backendPath.'frontendMenus/manage">'.A::t('app', 'Back').'</a>' : A::t('app', 'Frontend Menu'));
	?>
	</div>
    <div class="content">
    <?php 
		...
		if($menuType == 'moduleblock'){
			...
			if(Admins::hasPrivilege('frontend_menu', 'edit')){
				echo '<a href="'.$backendPath.'frontendMenus/add'.($parentId ? '/pid/'.(int)$parentId : '').'" class="add-new">'.A::t('app', 'Add New').'</a>';
			}		
					
			echo CWidget::create('CGridView', array(
				'model'				=> 'FrontendMenus',
				'actionPath'		=> $backendPath.'frontendMenus/manage'.($parentId ? '/pid/'.(int)$parentId : ''),
				...
				'fields'			=> array(
					...
					'sort_order'    => array('title'=>A::t('app', 'Sort Order'), 'type'=>'label', 'class'=>'center', 'headerClass'=>'center', 'width'=>'90px', 'changeOrder'=>true),
					'is_active' 	=> array('title'=>A::t('app', 'Active'), 'type'=>'link', 'align'=>'', 'width'=>'90px', 'class'=>'center', 'headerClass'=>'center', 'isSortable'=>false, 'linkUrl'=>$backendPath.'frontendMenus/changeStatus/id/{id}/page/{page}', 'linkText'=>'', 'definedValues'=>array('0'=>'<span class="badge-red">'.A::t('app', 'No').'</span>', '1'=>'<span class="badge-green">'.A::t('app', 'Yes').'</span>'), 'htmlOptions'=>array('class'=>'tooltip-link', 'title'=>A::t('app', 'Click to change status'))),
					'sub_menus_link'=> array('title'=>A::t('app', 'Sub-Menus'), 'type'=>'link', 'class'=>'center', 'headerClass'=>'center', 'width'=>'90px', 'isSortable'=>false, 'linkUrl'=>($parentId == 0 ? $backendPath.'frontendMenus/manage/pid/{id}' : ''), 'linkText'=>($parentId == 0 ? A::t('app', 'Sub-Menus') : A::t('app', 'N/A')), 'htmlOptions'=>array('class'=>'subgrid-link'), 'prependCode'=>($parentId == 0 ? '[ ' : ''), 'appendCode'=>($parentId == 0 ? ' ]' : ''), 'disabled'=>($parentId == 0 ? false : true)),
				...
				'actions'			=> array(
					'edit' => array(
						...
						'link'		=> $backendPath.'frontendMenus/edit/id/{id}'.($parentId ? '/pid/'.(int)$parentId : ''), 'imagePath'=>'templates/backend/images/edit.png', 'title'=>A::t('app', 'Edit this record')
					),
					'delete' => array(
						'disabled'	=> !Admins::hasPrivilege('frontend_menu', 'edit'),
						'link'		=> $backendPath.'frontendMenus/delete/id/{id}/pid/'.($parentId ? (int)$parentId : '0'), 'imagePath'=>'templates/backend/images/delete.png', 'title'=>A::t('app', 'Delete this record'), 'onDeleteAlert'=>true
					)
				),
			));			
		}		
