...
<div class="bloc">
	...
	<?php
		echo CHtml::openForm($backendPath.'settings/site', 'post', array('class'=>'widget-cformview'));
	...
	<fieldset>
		...
		<ul>
			...
			<li><strong><?= A::t('app', 'Alexa Rank'); ?>:</strong> <?= ($settings->alexa_rank == '' || $settings->alexa_rank == '0' ? '<i>'.A::t('app', 'Unknown').'</i>' : '<i>'.$settings->alexa_rank.'</i> &nbsp;<a href="http://www.alexa.com/siteinfo/'.$settings->website_domain.'" target="_blank" rel="noopener noreferrer"><img src="templates/backend/images/external_link.gif" title="'.A::te('app', 'Click to see').'"></a>'); ?></li>
		</ul>		
	</fieldset>
	<fieldset>
		<legend><?= A::t('app', 'Website Indexed Pages'); ?>:</legend>
		<ul>
			<li><strong><?= A::t('app', 'Google'); ?>:</strong> <?= ($settings->indexed_pages_google == '' || $settings->indexed_pages_google == '0' ? '<i>'.A::t('app', 'Unknown').'</i>' : '<i>'.$settings->indexed_pages_google.'</i>').' &nbsp;<a href="https://www.google.com/search?filter=0&q=site:'.$settings->website_domain.'" target="_blank" rel="noopener noreferrer"><img src="templates/backend/images/external_link.gif" title="'.A::te('app', 'Click to see').'"></a>'; ?></li>
			<li><strong><?= A::t('app', 'Bing'); ?>:</strong> <?= ($settings->indexed_pages_bing == '' || $settings->indexed_pages_bing == '0' ? '<i>'.A::t('app', 'Unknown').'</i>' : '<i>'.$settings->indexed_pages_bing.'</i>').' &nbsp;<a href="http://www.bing.com/search?scope=web&setmkt=en-US&setlang=match&FORM=W5WA&q=site:'.$settings->website_domain.'" target="_blank" rel="noopener noreferrer"><img src="templates/backend/images/external_link.gif" title="'.A::te('app', 'Click to see').'"></a>'; ?></li>
			<li><strong><?= A::t('app', 'Yahoo'); ?>:</strong> <?= ($settings->indexed_pages_yahoo == '' || $settings->indexed_pages_yahoo == '0' ? '<i>'.A::t('app', 'Unknown').'</i>' : '<i>'.$settings->indexed_pages_yahoo.'</i>').' &nbsp;<a href="https://search.yahoo.com/search?fr=sfp&p=site:'.$settings->website_domain.'" target="_blank" rel="noopener noreferrer"><img src="templates/backend/images/external_link.gif" title="'.A::te('app', 'Click to see').'"></a>'; ?></li>
			<li><strong><?= A::t('app', 'Yandex'); ?> (<?= A::t('i18n', 'countries.ru'); ?>):</strong> <?= ($settings->indexed_pages_yandex == '' || $settings->indexed_pages_yandex == '0' ? '<i>'.A::t('app', 'Unknown').'</i>' : '<i>'.$settings->indexed_pages_yandex.'</i>').' &nbsp;<a href="http://yandex.ru/yandsearch?text=site:'.$settings->website_domain.'&lr=10418'.'" target="_blank" rel="noopener noreferrer"><img src="templates/backend/images/external_link.gif" title="'.A::te('app', 'Click to see').'"></a>'; ?></li>
			<li><strong><?= A::t('app', 'Baidu'); ?> (<?= A::t('i18n', 'countries.cn'); ?>):</strong> <?= ($settings->indexed_pages_baidu == '' || $settings->indexed_pages_baidu == '0' ? '<i>'.A::t('app', 'Unknown').'</i>' : '<i>'.$settings->indexed_pages_baidu.'</i>').' &nbsp;<a href="http://www.baidu.com/s?ie=utf-8&wd=site:'.$settings->website_domain.'" target="_blank" rel="noopener noreferrer"><img src="templates/backend/images/external_link.gif" title="'.A::te('app', 'Click to see').'"></a>'; ?></li>
			<li><strong><?= A::t('app', 'Goo'); ?> (<?= A::t('i18n', 'countries.jp'); ?>):</strong> <?= ($settings->indexed_pages_goo == '' || $settings->indexed_pages_goo == '0' ? '<i>'.A::t('app', 'Unknown').'</i>' : '<i>'.$settings->indexed_pages_goo.'</i>').' &nbsp;<a href="http://search.goo.ne.jp/web.jsp?OE=UTF-8&mode=0&IE=UTF-8&MT=site:'.$settings->website_domain.'" target="_blank" rel="noopener noreferrer"><img src="templates/backend/images/external_link.gif" title="'.A::te('app', 'Click to see').'"></a>'; ?></li>
		</ul>		
	</fieldset>
	<fieldset>
