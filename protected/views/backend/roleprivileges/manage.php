<?php
    ...
    $this->_breadCrumbs = array(
        ...
        array('label'=>A::t('app', 'Roles & Privileges'), 'url'=>$backendPath.'roles/'),
...
<div class="bloc">
	<div class="title"><?= $role->name.' / '.A::t('app', 'Privileges'); ?> <a class="back-link" href="<?=$backendPath;?>roles/manage"><?= A::t('app', 'Back'); ?></a></div>
    <div class="content">
    ...
    <?php
		...
		if($role->code != 'owner'){
			$buttons = '<div class="buttons-wrapper">
				...
				<input name="" class="button white" onclick="$(location).attr(\'href\',\''.$backendPath.'roles/manage\');" value="'.A::t('app', 'Cancel').'" type="button" />
				</div>';			
		}

        echo CHtml::openTag('div', array('id'=>'privilegesSettings'));
		echo CHtml::openForm($backendPath.'rolePrivileges/manage/role/'.$id, 'post', array('name'=>'frmPrivileges', 'class'=>'widget-cformview'));
