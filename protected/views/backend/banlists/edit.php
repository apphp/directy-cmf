<?php
    ...
    $this->_activeMenu = $backendPath.'banLists/';
    $this->_breadCrumbs = array(
        array('label'=>A::t('app', 'General'), 'url'=>$backendPath.'dashboard/'),
        array('label'=>A::t('app', 'Ban Lists'), 'url'=>$backendPath.'banLists/manage'),
        array('label'=>A::t('app', 'Edit Item')),
    );    

	// Register mask plugin files
	A::app()->getClientScript()->registerScriptFile('assets/vendors/masked-input/jquery.mask.min.js', 2);	
?>
...
<div class="bloc">
    ...
    <?php
        ...
        $fields['expires_at'] = array('type'=>'datetime', 'title'=>A::t('app', 'Expires'), 'defaultAddMode'=>null, 'validation'=>array('required'=>false, 'type'=>'date', 'maxLength'=>10, 'minValue'=>date('Y-m-d'), 'maxValue'=>''), 'htmlOptions'=>array('maxlength'=>'10', 'style'=>'width:100px'), 'definedValues'=>array(), 'viewType'=>'date', 'dateFormat'=>'yy-mm-dd', 'timeFormat'=>'HH:mm:ss', 'buttonTrigger'=>true, 'minDate'=>'-30', 'maxDate'=>'', 'yearRange'=>'');
        ...
        echo CWidget::create('CDataForm', array(
			...
			'primaryKey'		=> $id,
            'operationType'		=> 'edit',
            'action'			=> $backendPath.'banLists/edit/id/'.$id,
            'successUrl'		=> $backendPath.'banLists/manage',
            'cancelUrl'			=> $backendPath.'banLists/manage',
			...
			'clientValidation'  => array('enabled'=>true, 'onlyRequired'=>true),
            'return'            => true,
        ));
    ?>        
    </div>
...
<?php
	A::app()->getClientScript()->registerScript(
		...
		'$("#frmBanListsEdit_item_type").on("change", function(){
			...
			unsetMask("frmBanListsEdit_item_value");
			if(itemType == "email_address"){
				...
				setIpAddressMask("frmBanListsEdit_item_value");
			}
		});		
		
		function unsetMask(el){
			$("#"+el).unmask();
		}
		function setIpAddressMask(el){
			$("#"+el).mask("0ZZ.0ZZ.0ZZ.0ZZ", {
				translation: {
					"Z": {
		...
		'.($itemType == 'ip_address' ? 'setIpAddressMask("frmBanListsEdit_item_value");' : ''),
		4
	);
