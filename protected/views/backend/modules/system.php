<?php
	...
	$this->_activeMenu = $backendPath.'modules/';
...
<div class="bloc">
    ...
    <div class="content">
		...
		<thead>
		<tr>
			<th class="left" colspan="2" style="width:120px;"><?= A::t('app', 'Module Name'); ?></th>
		...
		<?php 
			if(is_array($modulesList)){
				foreach($modulesList as $module){
					...
					$moduleDefaultUrl = !empty($backendDefaultUrl) ? $backendDefaultUrl : $backendPath.'modules/settings/code/'.$module['code'];
					if(strpos($moduleDefaultUrl, 'modules/settings') !== FALSE):
					...
					echo '<td class="left" width="84px">'.((Admins::hasPrivilege('modules', 'edit_management')) ? '<a href="'.$moduleDefaultUrl.'">'.A::t($module['code'], $module['name']).'</a>' : A::t($module['code'], $module['name'])).'</td>';
					...
					if(Admins::hasPrivilege('modules', 'edit_management')){
                        ...
                        if(!empty($moduleVersion) && $module['version'] < $moduleVersion) echo '<a href="'.$backendPath.'modules/update/code/'.$module['code'].'" class="tooltip-link" title="'.A::t('app', 'Update to version {version}', array('{version}'=>$moduleVersion)).'"><img src="templates/backend/images/update.png" alt="update"></a>';
                        echo '<a class="tooltip-link" title="'.A::t('app', 'Edit this record').'" href="'.$backendPath.'modules/edit/id/'.$module['id'].'"><img src="templates/backend/images/edit.png" alt="edit"></a>';
						if($removable){    
							echo '<a href="'.$backendPath.'modules/uninstall/id/'.$module['id'].'" class="tooltip-link" data-module="'.$module['name'].'" title="'.A::t('app', 'Uninstall this module').'" onclick="return onUninstallClick(this);"><img src="templates/backend/images/uninstall.png" alt="uninstall"></a>';
						}
                        echo '</td>';
					}
					echo '</tr>';
			...
			if(is_array($notInstalledModulesList)){
				foreach($notInstalledModulesList as $code => $module){
					echo '<tr>
						...
						<td class="left" width="84px">'.$module['name'].'</td>
					...
					if(Admins::hasPrivilege('modules', 'edit_management')){
						if(version_compare($frameworkVersion, $module['framework']) >= 0){
							...
							if(!empty($module['framework']) && $module['framework'] > A::app()->getVersion()){
								...
								echo '<a href="'.$backendPath.'modules/install/code/'.$code.'" title="'.A::te('app', 'Click to install this module').'">'.A::t('app', 'Install').'</a>';
							}
							echo '</td>';
						}else{
