...
class SetupController extends CController
{
	...
	public function readyAction()
	{
		...
		if($this->_cSession->get('step') < 4){
			...
			if(empty($sqlDump)){
				...
				$backendPath = isset($this->_configMain['defaultBackendDirectory']) ? $this->_configMain['defaultBackendDirectory'].'/' : '';
				...
				$sqlDump = str_ireplace(
					...
					array(
						$this->_cSession->get('dbPrefix'),
						$this->_cSession->get('username'),
						$salt,
						(($encryption) ? CHash::create($encryptAlgorithm, $this->_cSession->get('password'), $salt) : $this->_cSession->get('password')),
						$this->_cSession->get('email'),
						date('Y-m-d H:i:s', time() + (date('I', time()) ? 3600 : 0)),
						date('Y-m-d', time() + (date('I', time()) ? 3600 : 0)),
						date('Y-m-d H:i:s', time() + (date('I', time()) ? 3600 : 0)),
						$backendPath
					),
					$sqlDump
	...
	public function completedAction()
	{
		...
		$this->_view->backendPath = isset($this->_configMain['defaultBackendDirectory']) ? $this->_configMain['defaultBackendDirectory'].'/' : '';
