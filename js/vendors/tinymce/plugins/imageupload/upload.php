...
if(!CAuth::isLoggedInAsAdmin()){
    ...
    // order correctly
    if($getOrder == 'desc' && $getOrder != 'size') $dirs = @array_reverse($dirs); 
    if($getOrder == 'desc') $files = @array_reverse($files);
    $dirs = @array_values($dirs);
    $files = @array_values($files);
    
    $baseUrl = str_replace('js/vendors/tinymce/plugins/imageupload/', '', A::app()->getRequest()->getBaseUrl());
}
