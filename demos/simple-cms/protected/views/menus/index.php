	...
	<p>
		...
		if (empty($menus)) {
			...
			foreach ($menus as $cat) { ?>
				<tr>
					<td align="left" style="cursor:pointer;" onclick="window.location.href='menus/edit/id/<?= $cat['id'] ?>'" title="Click to edit"><?= $cat['name'] ?></td>
					<td align="center"><?= $cat['sort_order'] ?></td>
					<td align="center">
						<a href="menus/edit/id/<?= $cat['id'] ?>">Edit</a> |
						<a href="menus/delete/id/<?= $cat['id'] ?>" onclick="if(!confirm('Are you sure you want to delete this menu?\nNote: this will make all its menu links invisible to your site visitors!'))return false;">Delete</a>
					</td>
				</tr>
				<?php
			}
