...
class A
{
	...
	private function _autoload($className)
	{
		...
		elseif (isset(self::$_coreHelpers[$className])) {
			...
			// $classNameItems[n] - Type (Controller, Model, Entity, etc..)
	...
	private function _runApp()
	{
		...
		if (CConfig::get('db.driver') != '') {
			$db = CDatabase::init();
			if (CConfig::get('cache.db.enable') && !CAuth::isGuest()) {
				$db->cacheOff();
	...
	public function getComponent($id)
	{
		if (isset($this->_components[$id])) {
			...
			if ($this->_coreComponentsLazyLoading) {
				$componentFound = false;
				foreach (self::$_coreComponents as $componentId => $component) {
					if ($id == $componentId) {
	...
	{
		return !strncasecmp($name, '_on', 3) && CClass::isMethodExists($this, $name);
	}
	...
	{
		return array('ar', 'de', 'en', 'es', 'fr', 'he', 'it', 'nl', 'pl', 'ru');
	}
